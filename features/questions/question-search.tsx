  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-3 rounded-2xl border bg-card p-4 lg:grid-cols-[1fr_180px_180px_140px]">
        <Input placeholder={u.search.placeholder} value={query} onChange={(event) => setQuery(event.target.value)} />
        <select
          className="h-11 rounded-xl border bg-background px-3 text-sm"
          value={category}
          onChange={(event) => setCategory(event.target.value as QuestionCategory | "all")}
        >
          <option value="all">{u.search.allTopics}</option>
          {categories.map((item) => (
        ...
        >
          <option value="all">{u.search.allLevels}</option>
        ...
        >
          {favoritesOnly ? u.search.savedOnly : u.search.allSaved}
        </button>
      </div>
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <span>{u.search.results(results.length)}</span>
      </div>
